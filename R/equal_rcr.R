...
equal_rcr = function(nrow, ncol, rcr) {
  ...
  columns = t(permutations(n = 2, r = nrow_plus, v = c(0:cells), repeats.allowed = TRUE))
  ...
  multi_heatmaps = ggplot(data = df, mapping = aes(x = factor(Var2), y = factor(Var1))) +
    ...
    scale_fill_manual(values = c('1' = "#75e8ff", '0' = "#ffffff"), breaks = c("1"), labels = c("ROOFTOP")) +
    coord_equal() +
